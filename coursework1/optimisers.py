...
class Optimiser(object):
    ...
    def do_training_epoch(self):
        ...
        with self.tqdm_progress(total=self.train_dataset.num_batches) as train_progress_bar:
            ...
            for inputs_batch, targets_batch in self.train_dataset:
                ...
                print('op',self.learning_rule.learning_rate)
    ...
    def eval_monitors(self, dataset, label):
        ...
        for key, data_monitor in self.data_monitors.items():
            ...
            print('acc?',data_mon_vals[key + label])
    ...
    ##### two parameters for early stopping#######
            best_acc=0.
            epoch_count = 0.
            ...
            for epoch in range(1, num_epochs + 1):
                ...
                if self.scheduler is not None:
                    self.learning_rule.learning_rate = self.scheduler.update_learning_rule_restart(
                    learning_rule=self.learning_rule, epoch_number=epoch)
                
                
                print('train_epoch', self.learning_rule.learning_rate)
                ...
                if epoch % stats_interval == 0:
                    ...
                    print('single value?', list(stats.values()))
                    print('stats matrix', np.array(run_stats)[epoch,-1])
                    
                    ##############simple version of early stopping##############
                    this_acc = np.array(run_stats)[epoch,-1]
                    
                    
                    if this_acc >= best_acc:
                        best_acc = this_acc
                        best_param = copy.deepcopy(self.learning_rule.params)
...
#                             with open
        ...
        return np.array(run_stats), {k: i for i, k in enumerate(stats.keys())}, total_train_time, best_param
