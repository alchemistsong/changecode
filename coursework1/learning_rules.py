...
class AdamLearningRule(GradientDescentLearningRule):
    ...
    def update_params(self, grads_wrt_params):
        ...
        for param, mom_1, mom_2, grad in zip(self.params, self.moms_1, self.moms_2, grads_wrt_params):
            ...
            mom_1_biascorr = mom_1 / (1-self.beta_1**self.step_count)
            mom_2_biascorr = mom_2 / (1-self.beta_2**self.step_count)
            
            
            param -= (self.learning_rate * mom_1_biascorr / ((self.epsilon + mom_2_biascorr)**0.5))
#         self.step_count += 1
#         print(self.step_count)
#         self.step_count *= 0
...
class AdamLearningRuleWithWeightDecay(GradientDescentLearningRule):
    ...
    def __init__(self, learning_rate=1e-3, beta_1=0.9, beta_2=0.99,
                 epsilon=1e-8, weight_decay=0.00001, ηt=1):
        """Creates a new learning rule object.
        Args:
            learning_rate: A postive scalar to scale gradient updates to the
                parameters by. This needs to be carefully set - if too large
    ...
    def update_params(self, grads_wrt_params):
        ...
        #####check whether self.step_count should be reset to 0 or not####
        self.step_count += 1
        print(self.step_count)
#         print(self.step_count)
#         print(self.beta_2)
#         print(self.beta_2**self.step_count)
...
#             print(mom_2)
            ...
            print('update_para', self.learning_rate)
        
            param *= (1-self.learning_rate * self.weight_decay)
            param -= (self.initial_learning_rate * self.learning_rate * mom_1_biascorr / (self.epsilon + mom_2_biascorr**0.5))
...
class AdaGradLearningRule(GradientDescentLearningRule):
    ...
    def update_params(self, grads_wrt_params):
        ...
        for param, sum_sq_grad, grad in zip(
            ...
            self.sum_sq_grads.append(sum_sq_grad)
...
class RMSPropLearningRule(GradientDescentLearningRule):
    ...
    def initialise(self, params):
        ...
        self.sq_grad_accs = []
        for param in self.params:
            self.sq_grad_accs.append(np.zeros_like(param))
    ...
    def reset(self):
        ...
        """
        for sq_grad_acc in self.sq_grad_accs:
            sq_grad_acc *= 0.
        print('flag')
    ...
    def update_params(self, grads_wrt_params):
        ...
        Args:
            grads_wrt_params: A list of gradients of the scalar loss function
                ...
                previously, with this list expected to be in the same order.
        """
        for param, sq_grad_cc, grad in zip(self.params, self.sq_grad_accs, grads_wrt_params):
            
            sq_grad_cc *= self.beta
            sq_grad_cc += (1-self.beta)*(grad**2)
            param -= (self.learning_rate * grad / ((self.epsilon + sq_grad_cc)**0.5))
