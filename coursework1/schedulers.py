...
class CosineAnnealingWithWarmRestarts(object):
    ...
    def update_learning_rule(self, learning_rule, epoch_number):
        ...
        discount_factor = self.max_learning_rate_discount_factor ** (np.floor(epoch_number/self.total_epochs_per_period))
        
        indictor = epoch_number % self.total_epochs_per_period

        
        learning_rule.learning_rate = self.min_learning_rate + 0.5*(
            self.max_learning_rate*discount_factor-self.min_learning_rate)*(1.0+np.cos(indictor*np.pi/self.total_epochs_per_period))
    ...
    def update_learning_rule_restart(self, learning_rule, epoch_number):
        """Update the hyperparameters of the learning rule.
        ...
        if epoch_number <= self.total_epochs_per_period:
            
#             self.total_epochs_per_period = Ti
        ...
        if epoch_number > self.total_epochs_per_period:
            ...
            max_learning_rate=self.max_learning_rate * self.max_learning_rate_discount_factor
            
            total_epochs_per_period = self.total_epochs_per_period * self.period_iteration_expansion_factor
            
            discount_factor = self.max_learning_rate_discount_factor ** (
                np.floor((epoch_number - self.total_epochs_per_period) / total_epochs_per_period))
            
            indictor = (epoch_number - self.total_epochs_per_period ) % total_epochs_per_period
            
            learning_rule.learning_rate = self.min_learning_rate + 0.5*(
            max_learning_rate*discount_factor-self.min_learning_rate) * (1.0+np.cos(indictor*np.pi/total_epochs_per_period))
